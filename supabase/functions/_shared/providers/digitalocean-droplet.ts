...
export async function provisionDroplet(
	...
	}>("/droplets", { method: "POST", body: JSON.stringify(body) });

	const publicIp = created.droplet.networks?.v4?.find(
		(n) => n.type === "public",
	)?.ip_address;
...
export async function executeDropletLifecycle(
	args: LifecycleArgs,
): Promise<string> {
	...
	switch (args.action) {
		case "suspend":
			await apiRequest(`/droplets/${id}/actions`, {
				method: "POST",
		...
		default:
			throw new Error(
				`Unsupported lifecycle action '${args.action}' for droplets.`,
			);
	}
}

export async function syncDropletStatus(args: SyncArgs): Promise<SyncResult> {
	if (!/^\d+$/.test(args.providerResourceId))
	...
	const publicIp = result.droplet.networks?.v4?.find(
		(n) => n.type === "public",
	)?.ip_address;
