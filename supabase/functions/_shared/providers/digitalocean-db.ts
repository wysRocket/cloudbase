...
function extractConnection(conn?: Record<string, unknown>): Record<string, string> | undefined {
...
export async function provisionDb(args: ProvisionArgs): Promise<ProvisionResult> {
...
export async function executeDbLifecycle(args: LifecycleArgs): Promise<string> {
  ...
  throw new Error(`Unsupported lifecycle action '${args.action}' for managed databases.`);
}
