...
export default function Navbar() {
	...
	// Dynamic text colors
	const textColor =
		isLightPage && !scrolled ? "text-slate-900" : "text-slate-100";
	const hoverColor =
		isLightPage && !scrolled ? "hover:text-cyan-600" : "hover:text-cyan-400";
	const iconColor =
		isLightPage && !scrolled ? "text-slate-900" : "text-current";
	...
	return (
		<>
			...
			<nav
				className={`fixed top-0 w-full z-[100] px-4 md:px-6 transition-all duration-300 border-b border-white/5 ${scrolled ? "py-2 bg-black/80 backdrop-blur-xl" : "py-4 glass"}`}
			>
				<div className="max-w-7xl mx-auto flex justify-between items-center">
					...
					<Link to="/" className="flex items-center gap-2">
						...
						<span
							className={`text-xl font-bold tracking-tighter transition-colors ${textColor}`}
						>
							Cloudbase
						</span>
					...
					{/* Desktop Nav */}
					<div
						className={`hidden md:flex items-center gap-8 text-sm font-medium transition-colors ${textColor}`}
					>
						{navLinks.map((link) =>
							link.submenu ? (
								<div
						...
						>
							<div className="p-6 flex flex-col gap-4 text-slate-100">
								...
								<div className="border-t border-white/10 pt-2">
									<p className="text-cyan-400/60 text-xs uppercase tracking-wider mb-3 font-bold pl-2">
										Services
									</p>
