...
export default function Pricing() {
    const vpsPlans = [
        { name: 'Hobbyist', ram: '1GB', storage: '25GB', cpu: '1 vCPU', price: 5 },
        { name: 'Developer', ram: '2GB', storage: '50GB', cpu: '1 vCPU', price: 10 },
        { name: 'Startup', ram: '4GB', storage: '80GB', cpu: '2 vCPU', price: 20, popular: true },
        { name: 'Business', ram: '8GB', storage: '160GB', cpu: '4 vCPU', price: 40 },
        { name: 'Enterprise', ram: '16GB', storage: '320GB', cpu: '8 vCPU', price: 80 },
    ]
        ...
        <>
                    ...
                    <div className="grid md:grid-cols-5 gap-6">
                        {vpsPlans.map((plan, i) => (
                            <div key={i} className={`glass p-6 rounded-2xl ${plan.popular ? 'border-cyan-500/50 bg-cyan-950/20 scale-105' : ''}`}>
                                ...
                                <div className="text-3xl font-black mb-4">${plan.price}<span className="text-sm font-normal text-slate-500">/mo</span></div>
            ...
            <section className="py-16 px-6 bg-white/5">
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="grid md:grid-cols-3 gap-8">
                        {[
                            { name: 'Development', nodes: '1-3 nodes', price: 40 },
                            { name: 'Production', nodes: '3-10 nodes', price: 100, popular: true },
            ...
            <section className="py-16 px-6">
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="grid md:grid-cols-3 gap-8">
                        {[
                            { name: 'Starter', ram: '1GB', storage: '10GB', price: 15 },
                            { name: 'Professional', ram: '4GB', storage: '50GB', price: 50, popular: true },
                            { name: 'Business', ram: '16GB', storage: '200GB', price: 150 },
                        ].map((plan, i) => (
                            <div key={i} className={`glass p-8 rounded-2xl ${plan.popular ? 'border-cyan-500/50 bg-cyan-950/20' : ''}`}>
                                {plan.popular && <div className="text-xs bg-cyan-500 text-black px-2 py-1 rounded-full font-bold inline-block mb-3">Popular</div>}
                                <h3 className="text-2xl font-bold mb-4">{plan.name}</h3>
                                <div className="text-4xl font-black mb-4">${plan.price}<span className="text-lg font-normal text-slate-500">/mo</span></div>
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full glass rounded-2xl overflow-hidden">
                            ...
                            <tbody>
                                {[
                                    { name: 'NVIDIA T4', vram: '16GB', hourly: 0.50, monthly: 300 },
                                    { name: 'NVIDIA A10G', vram: '24GB', hourly: 1.00, monthly: 600 },
                                    { name: 'NVIDIA A100', vram: '40GB', hourly: 3.00, monthly: 1800 },
                                    { name: 'NVIDIA H100', vram: '80GB', hourly: 5.00, monthly: 3000 },
                                ].map((gpu, i) => (
                                    <tr key={i} className="border-b border-white/5 hover:bg-white/5">
                                        <td className="p-4 font-bold">{gpu.name}</td>
                                        <td className="p-4 text-slate-400">{gpu.vram}</td>
                                        <td className="p-4 text-cyan-400">${gpu.hourly}/hr</td>
                                        <td className="p-4 text-cyan-400">${gpu.monthly}/mo</td>
                                        <td className="p-4">
            ...
            <section className="py-16 px-6">
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="grid md:grid-cols-3 lg:grid-cols-6 gap-4">
                        {[
                            { name: 'Minecraft', price: 5 },
                            { name: 'Rust', price: 15 },
                            { name: 'CS2', price: 10 },
                            { name: 'ARK', price: 20 },
                            { name: 'Valheim', price: 8 },
                            { name: 'Terraria', price: 5 },
                        ].map((game, i) => (
                            <div key={i} className="glass p-6 rounded-xl text-center hover:bg-white/10 transition-all">
                                ...
                                <p className="text-cyan-400 font-bold">From ${game.price}/mo</p>
                            </div>
                        ))}
                    </div>
                </div>
