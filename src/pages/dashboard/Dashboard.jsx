...
export default function Dashboard() {
	const { resources, balance, transactions } = useDashboard();
	const { user } = useAuth();
	...
	const monthCreditsSpent = transactions
		.filter((tx) => tx.type === "debit" && tx.date.startsWith(currentMonth))
		.reduce((sum, tx) => sum + Math.abs(tx.amount), 0);
	...
	const stats = [
		{
			...
			change: resources.length > 0 ? "Operational" : "No services",
			trend: resources.length > 0 ? "up" : "neutral",
		},
		{
			title: "Current Balance",
			value: `${Math.floor(balance)} credits`,
			change: "Available funds",
			trend: "neutral",
		},
		{
			title: "Credits Spent (All Time)",
			value: `${Math.floor(totalCreditsSpent)} credits`,
			change: "Lifetime usage",
			trend: totalCreditsSpent > 0 ? "up" : "neutral",
		},
		{
			title: "Credits Spent (This Month)",
			value: `${Math.floor(monthCreditsSpent)} credits`,
			change: "Monthly consumption",
			trend: monthCreditsSpent > 0 ? "up" : "neutral",
		},
	];
	...
	return (
		<>
			...
			<div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4 mb-10">
				...
				<Link
					to="/dashboard/new"
					className="bg-cyan-600 hover:bg-cyan-500 text-white px-4 md:px-6 py-2 md:py-3 rounded-xl font-bold transition-all shadow-lg hover:shadow-cyan-500/25 flex items-center justify-center gap-2 text-sm md:text-base whitespace-nowrap"
				>
					<svg
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-10">
				{stats.map((stat, i) => (
					...
					>
						<h3 className="text-slate-400 text-xs md:text-sm font-medium mb-3">
							{stat.title}
						</h3>
						<div className="flex-1 flex flex-col justify-between gap-3">
							...
							<div
								className={`text-xs px-2 py-1 rounded-full self-start max-w-full ${
									stat.trend === "up"
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 mb-10">
				<motion.div
					...
					className="bg-[#0f1629] border border-white/5 rounded-2xl p-5 md:p-6 lg:col-span-1"
				>
				...
				<motion.div
					...
					className="bg-[#0f1629] border border-white/5 rounded-2xl p-5 md:p-6 lg:col-span-2"
				>
					<div className="flex items-center justify-between mb-4">
						<h2 className="text-lg font-bold">Credits Spending</h2>
						<Link
					...
					<div className="grid sm:grid-cols-2 gap-4">
						<div className="bg-white/5 border border-white/5 rounded-xl p-4">
							<p className="text-slate-400 text-xs mb-1">Lifetime Spending</p>
							<p className="text-2xl font-bold text-white">
								{Math.floor(totalCreditsSpent)}{" "}
								<span className="text-sm text-cyan-400">credits</span>
							</p>
							<p className="text-xs text-slate-500 mt-2">
								Total debits from all transactions
							</p>
						</div>
						<div className="bg-white/5 border border-white/5 rounded-xl p-4">
							<p className="text-slate-400 text-xs mb-1">This Month</p>
							<p className="text-2xl font-bold text-white">
								{Math.floor(monthCreditsSpent)}{" "}
								<span className="text-sm text-cyan-400">credits</span>
							</p>
							<p className="text-xs text-slate-500 mt-2">
								Debits where date starts with {currentMonth}
							</p>
						</div>
					</div>
				</motion.div>
